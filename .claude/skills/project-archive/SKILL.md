...
| **Follow-up** | Medium-size fix, related to this project but out of scope | Dispatch as a separate project or PR (suggest `/project-plan`) |
| **New project** | Large discovery — different scope, different substrate | Invoke `/project-plan <topic>` after archive PR is up |
...
- **Follow-up findings**: do **not** implement. Write a note in
  RETROSPECTIVE.md with a TODO link format suggesting `/project-plan`
  or a direct PR later.
- **New project**: after the archive PR is authored, invoke
  `/project-plan <topic>` to birth the replacement.
...
3. Invoke `/project-autosave` with `--event=archived
...
Invoke `/project-pull-request <slug> archive/<slug>` to author the
