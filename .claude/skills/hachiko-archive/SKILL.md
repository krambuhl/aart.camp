...
| **Follow-up** | Medium-size fix, related to this project but out of scope | Dispatch as a separate project or PR (suggest `/hachiko-plan`) |
| **New project** | Large discovery — different scope, different substrate | Invoke `/hachiko-plan <topic>` after archive PR is up |
...
- **Follow-up findings**: do **not** implement. Write a note in
  RETROSPECTIVE.md with a TODO link format suggesting `/hachiko-plan`
  or a direct PR later.
- **New project**: after the archive PR is authored, invoke
  `/hachiko-plan <topic>` to birth the replacement.
...
3. Invoke `/hachiko-autosave` with `--event=archived
...
Invoke `/hachiko-pull-request <slug> archive/<slug>` to author the
...
- For each Follow-up, surface the suggested command to the user
  (`/hachiko-plan` or a plain PR) — do not auto-dispatch.
- For each New project, invoke `/hachiko-plan <topic>` so the user can
