...
- `$1` (`<project-slug-or-path>`) — resolved like `/hachiko-autosave`.
...
The PR body is (substitute `{NN}` with the two-digit checkin number,
e.g. `07`):
...
6. Invoke `/hachiko-autosave` with `--event=pr-opened --detail=#<N>`
   and `--phase-update` reflecting the new PR number on the appropriate
   phase row.

### 5. Stale PR

1. **Commit pending work** (same as § 4 step 1).
2. **Push** any new commits: `git push origin <branch>` with the standard
   push-retry policy.
3. Author fresh title and body per § 3.
4. Update via `mcp__github__update_pull_request`.
5. Invoke `/hachiko-autosave` with `--event=pr-updated --detail=#<N>`.
...
Exactly one of four terminal states, one line:

```
pr: no-op, #<N> current @ checkin <NN>
pr: created #<N>, authored from checkin <NN>
pr: updated #<N>, re-authored from checkin <NN>
pr: drift, marker <NN_marker> ahead of latest checkin <NN_disk> — no action
```
