...
description: >-
  ...
  /griot-save-session in --from-checkin mode, which replays a checkin
...
**From-checkin mode.** `/griot-save-session` invokes the skill with
...
- Automatically from `/griot-save-session` for each `correction:`
...
`/griot-save-session` can roll up the count:
